# --- PATH FIX (no tocar) ---
import os, sys, pathlib
FILE = pathlib.Path(__file__).resolve()
PAGES_DIR = FILE.parent           # .../src/app/pages
APP_DIR = PAGES_DIR.parent        # .../src/app
SRC_DIR = APP_DIR.parent          # .../src
for p in (str(SRC_DIR), str(APP_DIR)):
    if p not in sys.path:
        sys.path.insert(0, p)
# ----------------------------

import streamlit as st
st.set_page_config(page_title="Query Interface", page_icon="🔎", layout="wide")
st.markdown("""
<style>
section[data-testid="stSidebar"] [data-testid="stSidebarNav"] a { text-transform: capitalize; }
button, [data-testid^="baseButton"] { font-family: 'Montserrat','Roboto',sans-serif !important; }
</style>
""", unsafe_allow_html=True)
from app.ui.theme import apply_theme
from app.views.query_interface import render

apply_theme()
render()
