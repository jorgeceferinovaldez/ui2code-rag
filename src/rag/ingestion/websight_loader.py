...
from src.config import websight_data_dir, ui_examples_dir, project_dir
...
class WebSightLoader:
    ...
    def load_websight_subset(self, max_examples: int = 1000) -> List[Dict[str, Any]]:
        """
        Load a subset of WebSight dataset
        For now, this creates sample data. In production, this would fetch from HuggingFace
        
        Args:
            max_examples: Maximum number of examples to load
            
        Returns:
            List of WebSight examples
        """
        try:
            ...
            print(f"Error loading WebSight data: {e}")
            return []
    
    def _create_sample_websight_data(self, num_samples: int) -> List[Dict[str, Any]]:
        """
        Create sample data in WebSight format for prototype
        Replace this with actual dataset loading in production
        """
...
<body class="bg-white">
    <div class="max-w-6xl mx-auto px-4 py-8">
        ...
        for i in range(min(num_samples, len(sample_templates) * 5)):
            ...
            # Create variations by modifying the template
            sample = {
    ...
    def _extract_text_from_html(self, html: str) -> str:
        """Extract readable text from HTML for search purposes"""
        try:
    ...
    def websight_to_documents(self, websight_data: List[Dict[str, Any]]) -> List[Document]:
        """
        Convert WebSight dataset entries to Document objects for RAG pipeline
        
        Args:
            websight_data: List of WebSight dataset entries
        ...
        for entry in websight_data:
            try:
                ...
                # Create document - Store metadata in the text for search
                ...
                # Store additional metadata as attributes for later access
                ...
                print(f"Error converting WebSight entry {entry.get('id', 'unknown')}: {e}")
                continue
    ...
    def save_html_examples(self, documents: List[Document]) -> int:
        """
        Save Document objects as HTML files in the examples directory
        ...
        print(f"Saved {saved_count} HTML examples to {self.examples_dir}")
        return saved_count
    
    def load_full_websight_pipeline(self, max_examples: int = 1000) -> List[Document]:
        """
        Complete pipeline: Load WebSight data and convert to Documents
        
        Args:
            max_examples: Maximum number of examples to process
            
        Returns:
            List of Document objects ready for RAG pipeline
        """
        try:
            print(f"Loading WebSight dataset (max {max_examples} examples)...")
            
            # Load WebSight data
            websight_data = self.load_websight_subset(max_examples)
            
            if not websight_data:
                print("No WebSight data available")
                return []
            
            print(f"Converting {len(websight_data)} entries to Documents...")
            
            # Convert to Documents
            documents = self.websight_to_documents(websight_data)
            
            # Save HTML files for reference
            self.save_html_examples(documents)
            
            print(f"WebSight pipeline complete: {len(documents)} documents ready")
            
            return documents
            
        except Exception as e:
            print(f"Error in WebSight pipeline: {e}")
            return []
