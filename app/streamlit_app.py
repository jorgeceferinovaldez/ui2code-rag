...
from src.config import corpus_dir, project_dir, temp_images_dir, generated_code_dir
...
def query_rag_system(
    ...
    try:
        ...
        if use_reranking:
            ...
            raw_results = pipeline.retrieve_with_metadata(
                query,
                top_k=top_k,
...
def ui_to_code_page():
    ...
    # Initialize agents
    visual_agent, code_agent = get_ui_agents()
    
    if not visual_agent or not code_agent:
        ...
        if st.button("🚀 Analyze Design & Generate Code", type="primary"):
            ...
            try:
                ...
                with st.spinner("Analyzing design with AI vision model..."):
                    analysis_result = visual_agent.analyze_image(str(temp_file_path))
                ...
                with st.spinner("Searching for similar code patterns..."):
                    patterns = code_agent.retrieve_patterns(analysis_result, top_k=top_k)
                ...
                with st.spinner("Generating clean HTML/Tailwind CSS code..."):
                    code_result = code_agent.generate_code(patterns, analysis_result, custom_instructions=custom_instructions)
                ...
                if generated_code:
                    ...
                    if save_results and generated_code:
                        try:
                            saved_path = code_agent.save_generated_code(code_result)
                            st.success(f"✅ Code saved to: {saved_path}")
                            
                            # Also save analysis
                            analysis_path = visual_agent.save_analysis_result(analysis_result)
    ...
    with st.expander("🔧 UI-to-Code System Status"):
        if code_agent:
            rag_status = code_agent.get_rag_status()
            st.json(rag_status)
        else:
            st.error("Code agent not available")
...
def main():
    ...
    page = st.sidebar.selectbox(
        "Choose a page",
        ["Query Interface", "UI to Code", "System Status", "Corpus Information"]
    )
    ...
    if page == "Query Interface":
        ...
        with col1:
            query = st.text_area(
                "Enter your query:",
                placeholder="Ask a question about the documents in your corpus...",
                height=100
            )
                ...
                with st.spinner("Searching..."):
                    ...
                    if response:
                        ...
                        for result in response["results"]:
                            with st.expander(f"Result #{result['rank']} - {result['citation']} (Score: {result['score']:.4f})"):
        ...
        if status:
            ...
            with col3:
                st.metric("Corpus Path", corpus["corpus_path"] if len(corpus["corpus_path"]) < 30 else f"...{corpus['corpus_path'][-27:]}")
